...
def read_list(sents, max_chars=None):
    ...
    :param max_chars: int, the number of maximum characters in a word, this
      parameter is used when the ptm is configured with CNN word encoder.
    :return:
    """
    dataset = []
    textset = []
...
class Embedder(object):
    ...
    def get_model(self):
        ...
        # ptm dir, but the path given in the config directory was an
        # absolute path.
        ...
        # In many cases, such as the publicly available English ptm,
        ...
        if not os.path.exists(config_path):
            raise FileNotFoundError("Could not find the ptm config in either the ptm directory "
                                    "or the default configs.  Path in config file: %s" % args2.config_path)

        with open(config_path, 'r') as fin:
            config = json.load(fin)

        # For the ptm trained with character-based word encoder.
        ...
        # For the ptm trained with word form word encoder.
        ...
        # configure the ptm to evaluation mode.
        model.eval()
        return model, config
